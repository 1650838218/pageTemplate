(function ($) {
    $.widget('ui.tagit', {
        options: {
            allowDuplicates: false,// 是否允许标签重复
            caseSensitive: true,
            fieldName: 'tags',
            placeholderText: null,// input框中的描述性文字
            readOnly: false,// 只读
            removeConfirmation: false,// true 表示按两次退格键才能删除标签
            tagLimit: null,// number 限制最多可以有多少个标签，null表示不限制
            availableTags: [],// 自动补全的数据源
            autocomplete: {},// autocomplete.source 映射到 availableTags
            showAutocompleteOnFocus: false,//在用户键入任何东西之前显示自动补全
            allowSpaces: false,
            singleField: false,// 用一个input保存所有标签
            singleFieldDelimiter: ',',// input中的标签的分隔符
            singleFieldNode: null,// input 输入框的jq对象
            animate: true,// 动画
            tabIndex: null,// 给input添加一个tabIndex的属性，并赋值
            ...
            onTagAdded: null,
            onTagRemoved: null,
            tagSource: null// `autocomplete.source` is the replacement for tagSource.
        },
        _create: function () {
            var that = this;
            ...
            if (this.options.readOnly) this.tagInput.attr('disabled', 'disabled');// 是否只读
            if (this.options.tabIndex) {// 添加tabIndex属性
                this.tagInput.attr('tabindex', this.options.tabIndex);
            }
            if (this.options.placeholderText) {// 添加提示信息
                this.tagInput.attr('placeholder', this.options.placeholderText);
            }
            if (!this.options.autocomplete.source) {// 输入时自动补全标签的方法，过滤
                this.options.autocomplete.source = function (search, showChoices) {
                    var filter = search.term.toLowerCase();
                    var choices = $.grep(this.options.availableTags, function (element) {
                        return (element.toLowerCase().indexOf(filter) === 0);// 只查询以filter开头的
                    });
